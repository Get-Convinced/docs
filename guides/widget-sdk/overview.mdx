...
| Own the entire interface | `ConvincedClient` and `client.createVoiceController()` | [Custom UI](/guides/widget-sdk/custom-ui) |
...
<Note>
Version `.3` adds session-owned headless voice, typed-input capture and retryable finalization. The companion backend is deployed. See [session ownership and migration](/guides/widget-sdk/events#session-owned-headless-voice).
</Note>

## Install

These guides target the published prerelease **`0.1.1-webmcp.3`**:

```bash
npm install --save-exact @convinced/widget-sdk@0.1.1-webmcp.3
```

Commit your lockfile. This version is under npm's `next` tag; the unversioned install selects `latest`, which remains `0.1.0` and lacks WebMCP and saved prompt administration. The [tagged source](https://github.com/Get-Convinced/convinced-widget-sdk/tree/v0.1.1-webmcp.3) matches these guides.
...
Applies to SDK `0.1.1-webmcp.3` and the hosted session finalization verified on September 21, 2026. SDK exports and deployed backend features are listed separately.

| Capability | Current status |
| --- | --- |
| Hosted widget configuration, session initialization and session end | Verified against the deployed Convinced backend |
| Session-owned headless voice and typed input | `client.createVoiceController()` captures turns; `client.endSession()` saves them through the Convinced session |
