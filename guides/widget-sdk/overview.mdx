...
**Enmovil's Transformation page uses the headless option:** its Enmo Buddy interface is custom React, with the SDK's voice controller and tool registry underneath. See [the Enmovil example](/guides/widget-sdk/custom-ui#enmovil-transformation-uses-headless). The quickstart demonstrates `managed-v2`, the supplied widget interface.
...
The September 14, 2026 audit checked the published package against the hosted services. A browser API being present in the package does not mean every corresponding backend endpoint is deployed.
